...
get_group_repos_cb (SeafDBRow *row, void *data)
{
    ...
    int status = seaf_db_row_get_column_int (row, 9);
    ...
    if (srepo != NULL) {
        if (vrepo_id) {
            ...
            const char *origin_repo_name = seaf_db_row_get_column_text (row, 10);
...
seaf_repo_manager_list_inner_pub_repos (SeafRepoManager *mgr)
{
    ...
    sql = "SELECT InnerPubRepo.repo_id, VirtualRepo.repo_id, "
        ...
        "VirtualRepo.origin_repo, VirtualRepo.path, i.status "
                                    ...
                                    GError **error)
{
    ...
    if (!is_org)
        ...
        sql = "SELECT OrgGroupRepo.repo_id, v.repo_id, "
              ...
              "LEFT JOIN RepoInfo i ON OrgGroupRepo.repo_id = i.repo_id, "
              "Branch WHERE group_id = ? AND "
